...
逻辑：服务端检测到"当前没有客服在前台查看"时才会推送（切到后台/锁屏也算不在线），避免你正开着后台还被反复打扰。
...
- **访客预填邮箱**：访客首次打开聊天窗口，会先弹出一个小表单要求填邮箱（称呼选填），提交后才能开始聊天，邮箱会显示在后台会话列表和聊天头部
- 退出登录：后台右上角 ⏻ 按钮

另外 `/api/upload` 接口是公开的（不需要登录）——因为访客端发文件也要用它，这个是设计上的取舍，它只接受文件上传返回一个链接，不会泄露任何聊天记录。

注意：登录 session 目前存在内存里，**重启 Node 服务后需要重新登录一次**（正常运营不重启的话没影响）。这是个人单用户工具的简化实现，密码是明文存在 `.env` 里做直接比对，`.env` 文件本身不会随代码分发，权限上要确保只有你自己能读（VPS默认root权限一般没问题）。
...
- 富文本/表情、消息已读回执
- 访客端多语言
- Web Push（配合 VAPID）替代 Bark，做成真正的浏览器原生推送
- 多客服账号（目前是单账号，多人协作需要扩展 users 表 + 每条消息记录是哪个客服发的）
- session 持久化到数据库，重启服务不掉登录态
- 邮箱验证码校验，防止访客乱填假邮箱
- 简单数据看板（会话量、响应时长统计）
- 离线自动留言表单
...
项目已新增 `ios/` SwiftUI 原生工程。登录、会话列表、聊天、图片/文件上传、常用语、标签备注和设置全部由 iOS 原生控件渲染，不使用 WebView。第一次启动只填写服务器域名，之后通过 `/api/...` 与 Node.js 服务通信。App 前台使用 SSE 实时事件流，后台和锁屏提醒使用 Bark。
...
- 安装前必须把新版 `server/server.js` 与 `server/db.js` 部署到 VPS 并重启 PM2。
- 完整步骤见 `ios/README.md`。
