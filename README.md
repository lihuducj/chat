...
访客插件会在浏览器本地保存高强度随机的 `visitorId + visitorSecret`。首次连接前就会生成身份，Socket.IO 每次自动重连都会读取最新身份；服务端同时保证同一访客最多只有一个进行中的会话。升级旧数据库时若检测到同一访客有多个 `open` 会话，只会把较旧的标记为“已结束”，不会删除会话或消息。邮箱只用于客服识别和联系，不作为恢复聊天历史的凭证，避免他人填写相同邮箱后读取旧记录。
