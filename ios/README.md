...
PWA Web Push 与原生 App 是两个不同身份，不能直接沿用。1.2.2 起 Bark 通知携带 `servicedesk://conversation/<会话ID>`，点击后会直接打开原生 App 并进入对应会话。当前版本号为 1.3.0（build 14）。消息正文可自动识别 `https://example.com`、`www.example.com` 等网址，点击后由 iOS 默认浏览器打开，同时保留长按选择与复制文字。

build 14 的图片预览接入了 iOS 原生 VisionKit 实况文本：点击“提取文字”后可直接选择并复制图片中的文字，图片仍支持双击缩放、双指缩放和自由拖动。iOS 18 及以上还可以点击“翻译成中文”，把整张图片识别出的内容翻译为简体中文并复制全部结果；iOS 17 保留文字提取与系统选择菜单。APP 最低系统版本为 iOS 17。整图翻译配置在页面创建时一次性建立，长 OCR 内容分段处理，翻译失败时留在结果页并允许重试。
