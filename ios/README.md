...
- 最低支持 iOS 17；iOS 18 以上可选系统自动翻译客户文字（原文仍保留；iOS 17 正常聊天和提取图片文字，但不显示整图翻译功能）
...
服务端已经发送 `X-Accel-Buffering: no` 并每 15 秒保活，重连时还会按事件游标补发断线期间的消息。大多数现有反向代理无需修改。如果 App 顶部一直是橙点、只能依靠兜底刷新，在站点 Nginx 配置中给事件接口增加：
...
PWA Web Push 与原生 App 是两个不同身份，不能直接沿用。1.2.2 起 Bark 通知携带 `servicedesk://conversation/<会话ID>`，点击后会直接打开原生 App 并进入对应会话。当前版本号为 1.3.0（build 20）。消息正文可自动识别 `https://example.com`、`www.example.com` 等网址，点击后由 iOS 默认浏览器打开，同时保留长按选择与复制文字。
...
build 16 在 APP 与网页客服台的聊天记录中加入按本地日期显示的日期分隔条。超过 360 个字符或 8 行的文字消息会自动折叠为最多 8 行，点按“查看全文”后在可滚动的小弹层中显示全部内容；完整内容仍支持引用、复制全文、选择部分文字以及客服消息撤回。

build 17 在 APP 聊天页左上角返回按钮加入“其他会话未读总数”，不把当前已打开会话重复计入；APP、网页客服台和访客插件统一使用醒目的独立引用块。会话资料新增“最近 IP”，服务端会优先读取反向代理传入的 `X-Forwarded-For` / `X-Real-IP` 并在访客重连时更新。

build 18 移除了会干扰 SwiftUI 导航栈的自定义侧滑手势兼容层，解决打开会话资料后连续返回出现界面错乱或卡死的问题。聊天页恢复系统原生返回按钮和侧滑手势，其他会话未读数字显示在系统返回区域旁边。

build 19 优化图片链路：相册原图先在后台线程缩放并自适应压缩到约 900KB，再执行上传；增加解码后的内存图片缓存、消息 ID 去重、弱网幂等重试与成功回查。服务器、网页后台和访客插件同步加入发送回执与安全重试，断线恢复历史时不会重复追加图片。
